...
## Update the kernel
When using the `sudo apt upgrade` command for an update, your kernel will automatically be updated to the latest stable version. If you want to try the latest unstable test kernel, [download the latest kernel source code](2.Linux%20Kernel.md#downloade_linux_code) and [manually compile and update the kernel](2.Linux%20Kernel.md#Native_and_cross_builds).
...
```
Next, [download the kernel source code](2.Linux%20Kernel.md#downloade_linux_code). It's important to note that you should have already completed the steps for either [native builds or cross-compilation](2.Linux%20Kernel.md#Native_and_cross_builds).
...
After the kernel has been built, refer to the methods described [Compile the kernel](2.Linux%20Kernel.md#Native_and_cross_builds) to install the kernel.
